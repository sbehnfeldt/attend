...
abstract class Token implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Attend\\Database\\Map\\TokenTableMap';
    ...
    /**
     * Initializes internal state of Attend\Database\Base\Token object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\Attend\Database\Token The current object (for fluent API support)
     */
    ...
    public function setCookieHash($v)
    {
        ...
        if ($this->cookie_hash !== $v) {
            $this->cookie_hash = $v;
            $this->modifiedColumns[TokenTableMap::COL_COOKIE_HASH] = true;
        }
    ...
    public function setAccountId($v)
    {
        ...
        if ($this->account_id !== $v) {
            $this->account_id = $v;
            $this->modifiedColumns[TokenTableMap::COL_ACCOUNT_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TokenTableMap::translateFieldName('CookieHash', TableMap::TYPE_PHPNAME, $indexType)];
            $this->cookie_hash = (null !== $col) ? (string)$col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TokenTableMap::translateFieldName('Expires', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->expires = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : TokenTableMap::translateFieldName('AccountId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = TokenTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Attend\\Database\\Token'), 0, $e);
        }
    }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Attend\Database\Token
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Attend\Database\Token The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TokenTableMap::COL_COOKIE_HASH)) {
            $criteria->add(TokenTableMap::COL_COOKIE_HASH, $this->cookie_hash);
        }
        if ($this->isColumnModified(TokenTableMap::COL_EXPIRES)) {
            $criteria->add(TokenTableMap::COL_EXPIRES, $this->expires);
        }
        if ($this->isColumnModified(TokenTableMap::COL_ACCOUNT_ID)) {
            $criteria->add(TokenTableMap::COL_ACCOUNT_ID, $this->account_id);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Attend\Database\Token (or compatible) type.
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Attend\Database\Token Clone of current object.
     * @throws PropelException
     */
