...
abstract class GroupPermissionQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'attend', $modelName = '\\flapjack\\attend\\database\\GroupPermission', $modelAlias = null)
    {
    ...
    public function findPk($key, ?ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = GroupPermissionTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    public function filterByPermissionId($permissionId = null, ?string $comparison = null)
    {
        if (is_array($permissionId)) {
            ...
            if (isset($permissionId['min'])) {
                $this->addUsingAlias(GroupPermissionTableMap::COL_PERMISSION_ID, $permissionId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($permissionId['max'])) {
                $this->addUsingAlias(GroupPermissionTableMap::COL_PERMISSION_ID, $permissionId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByGroup($group, ?string $comparison = null)
    {
        if ($group instanceof \flapjack\attend\database\Group) {
            ...
            throw new PropelException('filterByGroup() only accepts arguments of type \flapjack\attend\database\Group or Collection');
        }
    }
    ...
    public function filterByPermission($permission, ?string $comparison = null)
    {
        if ($permission instanceof \flapjack\attend\database\Permission) {
            ...
            $this
                ->addUsingAlias(GroupPermissionTableMap::COL_PERMISSION_ID, $permission->toKeyValue('PrimaryKey', 'Id'), $comparison);
