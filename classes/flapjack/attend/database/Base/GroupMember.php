...
abstract class GroupMember implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\flapjack\\attend\\database\\Map\\GroupMemberTableMap';
    ...
    /**
     * Initializes internal state of flapjack\attend\database\Base\GroupMember object.
     */
    ...
    public function setAccountId($v)
    {
        ...
        if ($this->account_id !== $v) {
            $this->account_id                                           = $v;
            $this->modifiedColumns[GroupMemberTableMap::COL_ACCOUNT_ID] = true;
        }
    ...
    public function hydrate(
        ...
        try {
            $col      = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : GroupMemberTableMap::translateFieldName(
                'Id',
            ...
            $col            = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GroupMemberTableMap::translateFieldName(
                'GroupId',
                TableMap::TYPE_PHPNAME,
                $indexType
            )];
            ...
            $col              = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GroupMemberTableMap::translateFieldName(
                'AccountId',
                TableMap::TYPE_PHPNAME,
                $indexType
            )];
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildGroupMemberQuery::create(null, $this->buildPkeyCriteria())->setFormatter(
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(GroupMemberTableMap::COL_ACCOUNT_ID)) {
            $criteria->add(GroupMemberTableMap::COL_ACCOUNT_ID, $this->account_id);
        }
     ...
     * objects.
     *
     * @param  object  $copyObj  An object of \flapjack\attend\database\GroupMember (or compatible) type.
     ...
     *
     * @param  bool  $deepCopy  Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \flapjack\attend\database\GroupMember Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    {
        if (null !== $this->aGroup) {
            $this->aGroup->removeGroupMember($this);
        }
        if (null !== $this->aAccount) {
            $this->aAccount->removeGroupMember($this);
        }
        $this->id            = null;
        $this->group_id      = null;
